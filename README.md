...
让用户说一句话（如"帮我分析 electronics 类目，推荐3个主推商品"），系统内的 **10 个智能体**像真正的运营团队一样分工协作——理解需求、分析数据、预测趋势、写文案、做方案——最后汇总成一份报告。
...
curl http://localhost:8085/api/v1/intent/recognize ^
  ...
  -d "{""user_message"":""帮我分析 electronics 类目，推荐3个主推商品""}"

# 全链路编排
curl http://localhost:8085/api/v1/chat ^
  -H "Content-Type: application/json" ^
  -d "{""user_message"":""帮我分析 electronics 类目，推荐3个主推商品"",""session_id"":""test"",""turn_number"":1}"
```
...
系统内置了 5 个类目的模拟数据（electronics, clothing, home, jewelry, sports），每个类目含 20 个商品的完整信息（价格、评分、销量、库存等）。启动后端后数据自动初始化。
