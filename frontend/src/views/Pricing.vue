...
async function run() {
  ...
  try { await store.analyzePricing(productId.value, p || { title: '商品'+productId.value, price: 99 }, category.value) }
...
<template>
  <div>
    <h2 style="font-size:20px;font-weight:700;color:#1a1a2e;margin-bottom:16px">定价策略</h2>
    <div class="card" style="margin-bottom:16px;display:flex;gap:12px;align-items:center;padding:16px 20px;flex-wrap:wrap">
      ...
      <button class="btn btn-primary" :disabled="loading" @click="run">{{ loading ? '分析中...' : '定价分析' }}</button>
    </div>
    <div v-if="error" style="color:#d32f2f;margin-bottom:16px">{{ error }}</div>
    <div v-if="store.pricing" class="card">
      <pre style="font-size:13px;line-height:1.8;white-space:pre-wrap">{{ store.pricing.for_display }}</pre>
    </div>
  </div>
</template>
